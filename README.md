...
│   ├── algorithms/
│   │   ├── gap_score.py              # Humanitarian gap calculator
│   │   ├── viability_predictor.py    # Medicine health checker
│   │   └── routing_agent.py          # Autonomous recommender
│   ├── data_sources/
│   │   ├── gdacs_fetcher.py          # Fetch live disaster data
│   │   ├── weather_api.py            # Get temperature context
│   │   └── synthetic_data.py         # Mock cold chain data (for hackathon)
│   └── models/
│       └── medicine_threshold.py     # WHO standards for each medicine type
│
├── frontend/
│   ├── app.py                         # Streamlit main app
│   ├── pages/
│   │   ├── dashboard.py              # Real-time visualization
│   │   ├── zone_detail.py            # Drill-down on specific zones
│   │   └── recommendations.py        # Autonomous agent output
│   └── assets/
│       └── styles.css                # Custom theming (optional)
│
├── data/
│   ├── sample_disasters.json         # Historical disaster scenarios
│   ├── medicine_types.json           # Vaccine/insulin/blood specs
│   └── synthetic_temperatures.csv    # Mock IoT sensor logs
│
└── docs/
    ├── ARCHITECTURE.md               # Technical deep-dive
    ├── API_DOCS.md                   # API endpoints
    └── ALGORITHMS.md                 # Math behind gap score + viability
```
...
```bash
git clone https://github.com/yourteam/helix.git
cd helix
python -m venv venv
...
**4. See It In Action**
- Dashboard loads live GDACS disaster data
- Synthetic medicine supply data simulates cold chain
- Gap scores calculate in real-time
- Recommendations flow automatically

**No API keys needed. No cloud setup. Everything free.**

---

## 📈 How We Built This in 3 Days

**Day 1: Core Intelligence**
- Integrated GDACS API
- Built gap score algorithm
- Created medicine viability predictor
- Tested with sample disaster data

**Day 2: Dashboard + Agent**
- Streamlit dashboard with maps
...
**Result:** A working system judges can click through. A story they'll remember.
